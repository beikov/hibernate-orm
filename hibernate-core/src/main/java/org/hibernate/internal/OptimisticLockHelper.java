...
public final class OptimisticLockHelper {
	...
	public static void forceVersionIncrement(Object object, EntityEntry entry, EventSource session) {
		...
		final Object cacheEntry = updateCacheItem( object, previousVersion, nextVersion, cacheKey, entry, persister, session );
		session.getActionQueue()
				.registerProcess( new CacheCleanupProcess( cacheKey, persister, previousVersion, nextVersion, lock, cacheEntry ) );
	}
